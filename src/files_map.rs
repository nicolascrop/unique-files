...
struct TargetFileBrowser {
    files: HashMap<String, String>,
    base_path: String,
    preserve_path: bool
}

impl TargetFileBrowser {
    pub fn new(base_path: &str, preserve_path: bool) -> TargetFileBrowser {
        TargetFileBrowser {
            files: HashMap::new(),
...
impl FileBrowser for TargetFileBrowser {
    fn on_file_entry(&mut self, path: &PathBuf) {
...
struct SourceFileBrowser<'b> {
    base_path: String,
    target: &'b mut TargetFileBrowser,
    on_progress: fn(path: &PathBuf)
}

impl<'b> SourceFileBrowser<'b> {
    pub fn new(base_path: &str, target: &'b mut TargetFileBrowser, on_progress: fn(path: &PathBuf)) -> SourceFileBrowser<'b> {
        SourceFileBrowser {
...
impl<'b> FileBrowser for SourceFileBrowser<'b> {
    fn on_file_entry(&mut self, path: &PathBuf) {
        let hash = self.get_hash(&path);
        ...
        debug!("copying file {:?} path {:?}", hash, path);
...
pub fn start_copy(
    ...
    let mut target = TargetFileBrowser::new(&target, preserve_path);
    target.init();

    for source in sources {
        let mut source_file_browser = SourceFileBrowser::new(&source, &mut target, on_progress);
